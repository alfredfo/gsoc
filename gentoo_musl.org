...
I also hope that more binary packages (*-bin) will become pure static binaries built with musl for ensuring compatibility with every setup, regardless of their libc implementation.
...
I think it's an awesome operating system because it gives the user so much control.
I run Gentoo/musl on my laptop and thinking about putting it on my espressobin router aswell.
