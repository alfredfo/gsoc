...
+ Patches/commits/pull requests etc. to upstream projects.
  ...
  the developers of these projects will most likely continue support for musl instead of the patches breaking when the code changes. 
+ Patches (.patch files) for Gentoo (gentoo.git).
...
+ Setting up an ideal working environment and learning the workflow of Gentoo will be done before the project starts, mostly during the "Community Bonding"-period.
  
Most of my project will be around porting the KDE desktop and application suite, starting with getting the desktop itselfd to run and later on porting the applications. 
