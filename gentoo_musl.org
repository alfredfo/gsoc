...
I will divide packages into three groups: builds, runs and works.
"Works" means runs correctly without apparent bugs.
** Week 1
...
** Week 2 & 3
   ...
   This means kde-plasma/plasma-meta will build and run.
** Week 4
   Making sure the KDE desktop runs correctly, polishing all runtime bugs.
   Also getting a web browser to run like Qutebrowser, which should just build and
   run when dev-qt/qtwebengine works. 
** Week 5 (and onward)
   Working out KDE Plasma bugs and porting other KDE applications.
   Starting with Dolphin, Konsole and Ark, then Gwenview, Kate, Okular and Kcalc if time allows it.
