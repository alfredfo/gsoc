...
On Gentoo Linux there are currently two supported libc's, namely
*glibc* (~sys-libs/glibc~) and *musl* (~sys-libs/musl~).

** The LLVM C Library
*The LLVM C Library*, or *LLVM libc* is a relatively new libc implementation part of the LLVM
project and created by the LLVM foundation.
...
support. This includes sanitizer instrumentation of user binaries
including the libc functions. It also includes a complete test suite
for testing the public interface and internal functions.

LLVM libc is also easily decomposed so users can replace C library
functionality easily, to for example get consistent math precision for ~libm~
...
*** Drawbacks of LLVM libc
The libc is still incomplete and lacks functionality one
