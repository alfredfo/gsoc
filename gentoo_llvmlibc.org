...
the operating system, as well as many other built-in C functions. More
specifically it provides an abstraction for the kernel's syscall
interface, and standard C functions such as ~printf~, ~malloc~,
~dlinfo~, and those part of ~math.h~. It also provides type
definitions like those found in ~stdint.h~.
